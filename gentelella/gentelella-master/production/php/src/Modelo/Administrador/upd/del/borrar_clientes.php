<?php
	...
	class borrar_clientes{
		...
		public function delete_cliente(){
			//Verificador que efectivamente tiene algun valor no nulo los ingresos efectuados
				if(isset($this->Id)){
					...
					if($this->Id != 0){
						$update="UPDATE cliente SET Estado= :estado Where idCliente= :id";
						...
						echo "<script>setTimeout(\"location.href = 'http://localhost/php/gentelella-master/production/php/administrador/Clientes.php';\",1500);</script>";	
					}
		...
		public function delete_cliente_usu(){
			//Verificador que efectivamente tiene algun valor no nulo los ingresos efectuados
				if(isset($this->Id)){
					$estado=0;
					
					if($this->Id != 0){
						$update="UPDATE cliente SET Estado= :estado Where Usuarios_idUsuarios= :id";
						$resultado=$this->db->prepare($update);
						$resultado->execute(array(":estado"=>$estado, ":id"=>$this->Id));
						if(!isset($resultado)){
						echo "error al agregar registro";
